...
class creonTrade():
    def __init__(self, fcm):
        ...
        self.client = MongoClient("mongodb+srv://parkjh:dkagh@cluster0.qart7.mongodb.net/myFirstDatabase?retryWrites=true&w=majority")
    ...
    def buyOrder(self, code, type, userId=None):
        ...
        for user in users:
            ...
            self.fcm.sendMessage("매수", "{}원에 자동으로 {}을(를) 매수했습니다".format(price, name), user['token'])
    ...
    def sellOrder(self, code, type, userId=None):
        ...
        for user in users:
            ...
            # 자동매매 알림 (주문요청 후에 넣어야 될 것)
            print("token값: ", user['token'])
            self.fcm.sendMessage("매도", "{}원에 자동으로 {}을(를) 매도했습니다".format(price, name), user['token'])

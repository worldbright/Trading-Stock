...
def before_request():
    ...
    g.client = MongoClient("mongodb+srv://parkjh:dkagh@cluster0.qart7.mongodb.net/myFirstDatabase?retryWrites=true&w=majority")
...
def info():
    ...
    if request.method == 'POST':
        if dayChartObj.datas[request.form['companyName']]:
            ...
            db = g.client.Project
            news = db.newsData.find({'회사명': {'$regex':request.form['companyName'],'$options':'i'}}).sort([('$natural',-1)]).limit(20)
